...
const getGroupAssets = async (groupName, cursor = '', previousData = []) => new Promise(resolve => {
    superagent('GET', `https://catalog.roblox.com/v1/search/items`)
        ...
        .then(async resp => {
            ...
            if (nextPageCursor) {
                print('debug', `Found ${data.length} assets in group "${groupName}" (${groupId}), fetching next page (${nextPageCursor})...`);
                await sleep(5 * 1000);
            ...
            print('debug', `Found ${finalData.length} assets in group "${groupName}" (${groupId})`);
            resolve(finalData);
        })
        .catch(async error => {
            if (
...
export const getGroupAssetsData = async ids => new Promise(resolve => {
    superagent('POST', `https://catalog.roblox.com/v1/catalog/items/details`)
        ...
        .catch(async error => {
            ...
            if (error.response.text.includes('Too many requests')) {
...
export const refreshGroupAssets = async (force = false) => {
    ...
    for (const asset of assets) {
        ...
        try {
            ...
            print('error', `Failed to hash asset ${asset.id}, skipping...`)
        }
    }
...
export const updateItemPrice = async (id, price = defaultPrice) => new Promise(resolve => {
    uploaderAccount.agent.post(`https://itemconfiguration.roblox.com/v1/assets/${id}/update-price`)
        ...
        .catch(async error => {
            ...
            if (error.response.text.includes('Flood Limit Exceeded')) {
...
export const updateAllDescriptions = async () => {
    for (const asset of Object.values(groupAssets)) {
        ...
        print('debug', `Updated item ${asset.id} description to newly generated one`);
...
export const uploadAsset = async (name, type = 11, buffer) => new Promise(resolve => {
    ...
    form.append('asset.png', buffer, 'asset.png.png');

    uploaderAccount.agent.post(`https://itemconfiguration.roblox.com/v1/avatar-assets/${type}/upload`)
        .set('content-type', form.getHeaders()['content-type'])
        .set('x-csrf-token', uploaderAccount.csrf)
        .send(form.getBuffer())
        .then(resp => {
            const {assetId: id} = resp.body;
